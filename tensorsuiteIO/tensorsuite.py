...
class TensorSuiteHeader:
    ...
    block_partitions: tuple[int, ...] | None = None
    nnz_block: int | None = None
    partition_sizes: tuple[tuple[int, ...], ...] | None = None
...
@dataclass
class BlockTensorData:
    header: TensorSuiteHeader
    block_coords: np.ndarray
    partition_sizes: tuple[np.ndarray, ...]
...
def _parse_block_tail(lines: list[str], num_modes: int, dimensions: tuple[int, ...]) -> tuple[tuple[int, ...], int, tuple[tuple[int, ...], ...]]:
    if len(lines) < num_modes + 1:
        raise ValueError("Block TensorSuite file is missing block grid and partition lines")
    parts = [int(x) for x in lines[0].strip().split()]
    if len(parts) != num_modes + 1:
        raise ValueError("Block size line must contain one block count per mode plus nnz_block")
    block_partitions = tuple(parts[:-1])
    nnz_block = parts[-1]
    if any(x <= 0 for x in block_partitions):
        raise ValueError("Block partition counts must be positive")
    if nnz_block < 0:
        raise ValueError("nnz_block must be nonnegative")
    partition_sizes: list[tuple[int, ...]] = []
    for mode, (line, expected_count, dimension) in enumerate(zip(lines[1:], block_partitions, dimensions)):
        values = tuple(int(x) for x in line.strip().split())
        if len(values) != expected_count:
            raise ValueError(f"Block partition line for mode {mode} has {len(values)} entries, expected {expected_count}")
        if any(x <= 0 for x in values):
            raise ValueError(f"Block partition sizes for mode {mode} must be positive")
        if sum(values) != dimension:
            raise ValueError(f"Block partition sizes for mode {mode} sum to {sum(values)}, expected {dimension}")
        partition_sizes.append(values)
    return block_partitions, nnz_block, tuple(partition_sizes)


def parse_tns_block_header(path: str | os.PathLike[str]) -> TensorSuiteHeader:
    base = parse_tns_header(path)
    with _as_path(path).open("r", encoding="utf-8") as f:
        lines = [f.readline() for _ in range(4 + base.num_modes + 1)]
    block_partitions, nnz_block, partition_sizes = _parse_block_tail(
        lines[4:],
        base.num_modes,
        base.dimensions,
    )
    return TensorSuiteHeader(
        base.magic,
        base.version,
        base.name,
        base.num_modes,
...
def read_element_tns(path: str | os.PathLike[str]) -> TensorData:
    header = parse_tns_header(path)
...
def _find_bundle_metadata(path: str | os.PathLike[str]) -> dict[str, Any] | None:
    path = _as_path(path)
    for candidate in [
        path.with_name(f"{path.stem}_metadata.json"),
        path.parent / f"{path.parent.name}_metadata.json",
    ]:
        if candidate.exists():
...
def _is_block_sparse(metadata: dict[str, Any] | None) -> bool:
    return bool(metadata and metadata.get("sparsity_type") == "block")


def read_block_tns(path: str | os.PathLike[str], metadata: dict[str, Any] | None = None) -> BlockTensorData:
    header = parse_tns_block_header(path)
    if metadata is not None:
        cross_check_header_metadata(header, metadata)
    index_base = int((metadata or {}).get("index_base", 0))
    if index_base not in (0, 1):
        raise ValueError("metadata.index_base must be 0 or 1")
    assert header.block_partitions is not None
    assert header.nnz_block is not None
    assert header.partition_sizes is not None
    start_line = 4 + header.num_modes + 1
    block_coords: list[tuple[int, ...]] = []
    with _as_path(path).open("r", encoding="utf-8") as f:
        for _ in range(start_line):
            next(f)
        for lineno, line in enumerate(f, start=start_line + 1):
            s = line.strip()
            if not s:
                continue
            parts = s.split()
            if len(parts) != header.num_modes:
                raise ValueError(f"Invalid block entry line {lineno}: expected {header.num_modes} block coordinates")
            try:
                coord = tuple(int(x) for x in parts)
            except ValueError as exc:
                raise ValueError(f"Invalid block entry line {lineno}: {s}") from exc
            for axis, (value, upper) in enumerate(zip(coord, header.block_partitions)):
                lo = index_base
                hi = index_base + upper - 1
                if value < lo or value > hi:
                    raise ValueError(f"Block coordinate out of range at line {lineno}, mode {axis}: {value}")
            block_coords.append(coord)
    if len(block_coords) != header.nnz_block:
        raise ValueError(f"Block nnz mismatch: header={header.nnz_block}, actual={len(block_coords)}")
    coord_arr = np.asarray(block_coords, dtype=np.uint64).reshape((len(block_coords), header.num_modes))
    partition_arrays = tuple(np.asarray(parts, dtype=np.uint64) for parts in header.partition_sizes)
    return BlockTensorData(header=header, block_coords=coord_arr, partition_sizes=partition_arrays)


def read_tns(path: str | os.PathLike[str], metadata: dict[str, Any] | None = None) -> TensorData | BlockTensorData:
    metadata = metadata if metadata is not None else _find_bundle_metadata(path)
    if _is_block_sparse(metadata):
        return read_block_tns(path, metadata=metadata)
    return read_element_tns(path)
...
def parse_tnsb_block_header(path: str | os.PathLike[str]) -> TensorSuiteHeader:
    path = _as_path(path)
    with path.open("rb") as f:
        lines: list[bytes] = []
        for _ in range(4):
        ...
        for _ in range(num_modes + 1):
            line = f.readline()
            if not line:
                raise ValueError("Unexpected EOF while reading TNSB block header")
            block_lines.append(line.decode("utf-8"))
        payload_offset = f.tell()
    block_partitions, nnz_block, partition_sizes = _parse_block_tail(block_lines, num_modes, dims)
    return TensorSuiteHeader(
        magic,
        version,
        name,
...
def cross_check_header_metadata(header: TensorSuiteHeader, metadata: dict) -> None:
    ...
    if metadata.get("sparsity_type") == "block":
        if header.block_partitions is None or header.nnz_block is None:
            raise ValueError("block metadata requires a block TensorSuite header")
        if list(header.block_partitions) != list(metadata.get("block_partitions", [])):
            raise ValueError("header.block_partitions != metadata.block_partitions")
        if header.nnz_block != metadata.get("nnz_block"):
            raise ValueError("header.nnz_block != metadata.nnz_block")
